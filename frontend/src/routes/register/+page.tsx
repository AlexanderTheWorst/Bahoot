...
export default function () {
    ...
    return <>
        <div className={"font-dm-sans! absolute z-[1] top-0 left-0 w-screen h-screen flex items-center justify-center"}>
            <form method={"POST"} action={`${protocol}//${config.API_URI}/auth/register`}
                ...
                <button
                    className={"rounded-[10px] cursor-pointer hover:bg-dark-accent-secondary text-[18px] text-center bg-dark-accent-primary p-[10px]"}>Register
                </button>

                {/* OR */
                }
                <div>
                    <span>Already have an account? <Link to={"/login"} className={"underline-offset-4 underline"}>Login here</Link>!</span>
                </div>
            </form>
        </div>
    </>
